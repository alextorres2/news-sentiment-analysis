...
def get_articles_content(url: str) -> list:
    """Given a URL to NPR news homepage, get the text of all articles in the homepage
    ...
    for article in articles:
        if not article.get('href'):
            continue
        url = article['href'] if article['href'].startswith('https://') else 'https://www.npr.org' + article['href']
        sub_urls.add(url)
    ...
    for url in sub_urls:
        try:
            ...
            logger.error(f"Could not load {url}: {ex}")
            continue
...
def npr_homepage_articles_analysis(show: bool = False, save: bool = True):
    """Perform and save sentiment analysis on articles on CNN homepage website.
        ...
        Saves data to data/processed/npr/TextBlob/ and data/processed/npr/WordCloud/
    """
    ...
    try:
        ...
        stopwords = nltk.corpus.stopwords.words('english')
    extra_wordstoignore = ['npr', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'said', 'caption', 'permissions', 'page', 'images', 'hide', 'getty', 'programming', 'audio', 'rights', 'reserved', 'visit', 'website', 'org', 'information', 'terms', 'use', 'copyright', 'right', 'transcripts', 'created', 'pages', 'authoritative', 'updated', 'revised', 'contractor', 'text', 'news', 'may', 'deadline', 'form', 'record']
    ...
    if show:
        ...
        freq_dist.plot(20)
        plt.title('CNN Frequency Distribution')
    ...
    if save:
        ...
        now = now.replace(" ", "_")
        df_textblob.to_csv(f"data/processed/npr/TextBlob/textblob_{now}.csv")
        df_freq_dist.to_csv(f"data/processed/npr/WordCloud/freq_dist_{now}.csv")
