...
def get_articles_content(url: str) -> list:
    ...
    articles = soup.find_all('a', class_="item__anchor")
    ...
    for url in sub_urls:
        ...
        paragraphs = [p for p in paragraphs if not p in ['Watch CBS News', 'Copyright ©2025 CBS Interactive Inc. All rights reserved.', '© 2025 CBS Interactive Inc. All rights reserved.']]
        if not paragraphs:
            continue
...
def cbs_homepage_articles_analysis(show: bool = False, save: bool = True):
    ...
    try:
        ...
        stopwords = nltk.corpus.stopwords.words('english')
    extra_wordstoignore = ['cbs','1','2','3','4','5','6','7','8','9','0','said','says','also','one','two','three','four','five','six','seven','eight','nine']
    ...
    if show:
        ...
        plt.title('CBS Website WordCloud')
        plt.axis('off')
    ...
    if save:
        ...
        now = now.replace(" ", "_")
        df_textblob.to_csv(f"data/processed/cbs/TextBlob/textblob_{now}.csv")
        df_freq_dist.to_csv(f"data/processed/cbs/WordCloud/freq_dist_{now}.csv")
        logger.info(f"Saved data as CSV file for {url}")
    return

if __name__ == "__main__":
    cbs_homepage_articles_analysis(show=True, save=False)
